...
class AzureIoTHub {
  ...
  Future<String> callDirectMethod({
    ...
    /// Call the direct method specified by [methodName] on the device with [deviceID].
  /// Returns the response from the device.
  Future<String> iotHubQuery({
    @required String query,
  }) async {
    _prepareForRequest();
    var uri =
        "https://$iotHubEndpoint/devices/query?api-version=$API_VERSION";
  ...
  Future<List<Map<String, dynamic>>> retrieveDevicesList() async {
    final results = await this.iotHubQuery(query: "SELECT * FROM devices");
    return jsonDecode(results).cast<Map<String,dynamic>>();
  }
